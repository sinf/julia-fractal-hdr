...
def blit2(dst, src, dst_y=0, dst_x=0):
    rows,cols = src.shape[:2]
    dst[dst_y:dst_y+rows , dst_x:dst_x+cols, :] += src
...
def bluenoise(buf):
    n = cv2.imread('image/blue.png').astype('float64') - 127.5
    n = n*4
    h,w = buf.shape[:2]
    nh,nw = n.shape[:2]
...
def format_output(buf):
    ...
    if True:
        temp=np.zeros((buf.shape[0]*3, buf.shape[1], buf.shape[2]), dtype='float64')

        blit2(temp, buf, dst_y=0)
        blit2(temp, noise(buf.copy()), dst_y=buf.shape[0])
        blit2(temp, bluenoise(buf.copy()), dst_y=buf.shape[0]*2)

        temp = np.clip(temp/256.0,0,255).astype('uint8')
        temp = temp.astype(int)*20
        temp = np.clip(temp,0,255)
        temp = temp.astype('uint8')
        cv2.imwrite('test2.png', temp )
